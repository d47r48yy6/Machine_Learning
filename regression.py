...
housing = fetch_california_housing()    #housing is the object that contains the dataset
df =pd.DataFrame(housing.data, columns=housing.feature_names)

#Add the target variabale to the dataframe
...
#Train the model om the training data
...
#plot the actual data points  (Blue dots are the data points)
plt.scatter(X_test, y_test, color='blue', label='Actual')
#plot the regression line      (Red line is the regression line)
plt.plot(X_test, y_pred, color='red', label='Regression Line')
#Add lalbels and the title 
plt.xlabel('Median income(MedInc)')
plt.ylabel('House Price ($100,000)')
plt.title('Simple Linear Regression : House Price vs Median Income')
plt.legend() # Add the legend tp the plot 
plt.show()
